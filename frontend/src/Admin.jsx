...
const calcularFaltas = (aluno) => {
  ...
  if (aluno.se_ausenta_sempre || aluno.justificou_ausencia)
    return { totalAulas, presencas, faltas: 0, justificado: true };
  const abonos = aluno.saldo_abonos || 0;
  return {
    totalAulas,
    presencas,
    faltas: abonos > 0 ? Math.max(0, brutas - abonos) : brutas,
    justificado: false,
  };
};
...
const StatusPill = ({ aluno }) => {
  if (!aluno.formacao)
    return <span className="status-pill neutral">Sem turma</span>;
  const { faltas, totalAulas, justificado } = calcularFaltas(aluno);
  if (justificado)
    return <span className="status-pill info">✓ Justificado</span>;
  const pct = totalAulas > 0 ? (faltas / totalAulas) * 100 : 0;
  if (pct === 0) return <span className="status-pill ok">✓ Regular</span>;
  if (pct <= 25)
    return <span className="status-pill warn">⚠ {faltas} falta(s)</span>;
...
export default function Admin({ user, setView }) {
  ...
  const [resumoDup, setResumoDup] = useState({
  ...
  const carregarDuplicados = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setResumoDup({
          totalEmailsDuplicados: d.totalEmailsDuplicados || 0,
          totalRegistrosDuplicados: d.totalRegistrosDuplicados || 0,
        });
      }
    } catch {
    } finally {
  ...
  const removerDuplicado = async (reg) => {
    ...
    if (!confirm(`Remover registro duplicado de ${reg.nome || reg.email}?`))
      return;
    setRemovendoId(reg.id);
    try {
      const res = await fetchComToken(
        `/admin/aluno-id/${encodeURIComponent(reg.id)}`,
        "DELETE",
      );
      if (res.ok) {
        alert("Removido!");
  ...
  const buscarAlunos = useCallback(
    async (termo) => {
      ...
      try {
        const res = await fetchComToken(
          `/admin/busca?termo=${encodeURIComponent(termo)}&turma=${filtroTurma}&status=${filtroStatus}&dataFiltro=${dataBusca}&_=${Date.now()}`,
        );
        if (res.ok) {
          const d = await res.json();
          setAlunos(d.alunos || []);
  ...
  const salvarEdicao = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/admin/aluno/${encodeURIComponent(modalAluno.email)}`,
        {
          method: "PUT",
          headers: {
  ...
  const excluirAluno = async () => {
    if (
      !confirm(
        `ATENÇÃO: Excluir permanentemente ${modalAluno.nome || modalAluno.email} e todo seu histórico?`,
      )
    )
      return;
    setLoadModal(true);
  ...
  const exportarCSV = async () => {
    ...
    try {
      ...
      const rows = dados
        .map(
          (r) =>
            `${r.Nome};${r.Email};${r.Formacao};${r.Data};${r.Entrada};${r.Saida};${r.Nota};"${(r.Feedback || "").replace(/"/g, '""')}"`,
        )
        .join("\n");
    ...
    <div className="admin-wrapper">
        ...
        {[
          {
            ...
            val: stats.sessoesAtivas,
            cor: "var(--accent)",
          },
          {
            label: "Pendentes Saída",
            val: stats.pendentesSaida,
            cor: "var(--warning)",
          },
          {
            label: "Concluídos",
            val: stats.concluidosHoje,
            cor: "var(--success)",
          },
          {
            label: "Total Alunos",
            val: stats.totalAlunos,
            cor: "var(--text-main)",
          },
        ].map((s, i) => (
          <div key={i} className="admin-stat">
            <div className="admin-stat-label">{s.label}</div>
            <div className="admin-stat-value" style={{ color: s.cor }}>
              {s.val}
            </div>
          </div>
        ))}
            ...
            <div style={{ overflowX: "auto" }}>
              {alunos.length === 0 ? (
                ...
                >
                  {carregando
                    ? "Buscando..."
                    : "Nenhum resultado. Use os filtros acima."}
                </div>
              ) : (
                    ...
                    {alunos.map((aluno, i) => {
                      ...
                      return (
                        <tr key={i}>
                          <td>
                            <div
                              className="fw-bold text-sm truncate"
                              style={{ maxWidth: 160 }}
                            >
                              {aluno.nome || "—"}
                            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div className="shadow-card p-20">
            ...
            <div style={{ textAlign: "center", marginBottom: 12 }}>
              ...
              <div className="text-xs text-muted">
                {stats.sessoesAtivas} de {stats.totalAlunos} alunos
              </div>
            </div>
          ...
          >
            ...
            <div className="text-xs text-muted mb-12">
              {resumoDup.totalEmailsDuplicados} e-mail(s) ·{" "}
              {resumoDup.totalRegistrosDuplicados} registros
            </div>
            {duplicados.length === 0 ? (
              ...
              >
                {duplicados.map((grupo) => (
                  ...
                  >
                    <div
                      className="fw-bold text-xs mb-8 truncate"
          ...
          >
            ...
            <p className="text-xs text-muted">
              Use o filtro "Pendente Saída" para identificar quem não fez
              check-out na última aula.
            </p>
          </div>
        </div>
      </div>
      ...
      {modalAluno && (
        <div className="modal-overlay">
          <div
            className="modal-content shadow-xl"
          ...
          >
            ...
            <div className="tab-list mb-16">
              {[
                ...
                <button
                  key={t.key}
                  className={`tab-item ${tabModal === t.key ? "active" : ""}`}
            ...
            <div className="flex gap-8 mb-16" style={{ flexWrap: "wrap" }}>
              ...
              <span className="status-pill neutral">
                {calcularFaltas(modalAluno).presencas}/
                {calcularFaltas(modalAluno).totalAulas} presenças
              </span>
            </div>
                    ...
                    <tbody>
                      {historicoModal.map((h, i) => (
                        <tr key={i}>
                          ...
                          <td>
                            <span
                              className={`status-pill ${h.check_in && h.check_out ? "ok" : "warn"}`}
                            >
